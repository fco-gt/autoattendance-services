...
import attendanceRoutes from "./routes/attendance.router"; // Importará rutas de asistencia
...
if (!PORT || !API_VERSION) {
  logger.error("Faltan variables de entorno. Attendance Service no iniciará.");
  process.exit(1);
}
...
app.get("/", (req: Request, res: Response) => {
  res.send(`Attendance Service Base - API Path: ${API_BASE_PATH}`);
});
app.get(API_BASE_PATH, (req: Request, res: Response) => {
  res.send("Attendance Service API");
});

app.use(`${API_BASE_PATH}/attendance`, attendanceRoutes); // Montará rutas de asistencia
